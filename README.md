<img src="./images/github-header2.png"></img>

## Welcome ☺️

I'm Chris Lee -- A Product Manager and software builder with backgrounds in design, product development, and computer science. 

Most recently, I've been leading the development of HistoWiz's (YC16) PathologyMap, an ai-driven multimodal research platform that orchestrates big data pipelines and automates scientific workflows for thousands of researchers working to solve cancer.

Driven by a curious enthusiasm for everything around me, I love to find and build ideas with my life's goal to learn and apply all of the tools I need to create them. Over the last decade, I've worked across Vancouver, Shanghai, New York City, and San Francisco with a career spanning architecture, construction, industrial design, brand-building, and software. 

I operated thesis corp, a residential design + build studio ($60M in built work), toured as a punk-rock bassist releasing 2 EP's, build and race motorcycles, fly small planes, and co-founded thesis labs, an award-winning regulated PPE motorcycle tech-wear brand. This hands-on background still shapes how I think about systems, execution, and product quality.

I’m especially interested in products that sit at the intersection of software, design, science, and real-world utility.

#### Current WIP Projects 🤕

[Moto Ref <small><i>v1.02</i></small>](https://chrislee.wtf/experiments/moto-ref): An AI-native system combining RAG & vector search, OCR, and computer vision to enable real-time diagnostics and workflow automation for mechanics and hobbyists on mobile devices. Designed end-to-end pipeline including ingestion, embedding, retrieval, and multimodal reasoning, exploring agent orchestrationm and real-world tool usage. To be released Q3 2026. 

[Personal Portfolio <small><i>v1.21</i></small>](https://chrislee.wtf/): Fully responsive, coded from scratch portfolio when I have time.

[RideCast <small><i>2026-07-11</i></small>](https://www.chrislee.wtf/ridecast) Weather along your motorcycle route, timed to your ETA. Paste a Google Maps link, pick a departure time, and see conditions for every stretch of the ride.

[NetworkGraph <small><i>2026-07-11</i></small>](https://www.chrislee.wtf/experiments/networkgraph) A lightweight planning tool for turning roadmaps into dependency maps that show what unlocks what.

[Make School Archives](https://makeschool.fail): A conservatory for everything Make School as of July 2021.

#### Languages and Tools

<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/javascript/javascript.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/typescript/typescript.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/python/python.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/go/go.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/react/react.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/28b02bbc9ad9f7a503c43775aebeb515dc2da5fc/topics/nextjs/nextjs.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/redux/redux.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/nodejs/nodejs.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/html/html.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/css/css.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/tailwind/tailwind.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/mongodb/mongodb.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/postgresql/postgresql.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/git/git.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/78df643247d429f6cc873026c0622819ad797942/topics/github/github.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/graphql/graphql.png"></code>
<code><img height="30" src="https://raw.githubusercontent.com/github/explore/80688e429a7d4ef2fca1e82350fe8e3517d3494d/topics/docker/docker.png"></code>
